...
답변 규칙 (detailed 모드):
- 1000자 이내의 완결된 답변을 작성해요.
- 헤더(#), 목록(-, •), 표 등 구조가 도움된다면 자유롭게 사용해요.
- 1000자로 전달해야 할 내용의 50% 미만밖에 커버하지 못할 경우:
  1) 1000자 이내의 핵심 요약을 먼저 완결성 있게 작성하고 (이때도 반드시 완결된 문장으로 끝낼 것)
  2) 바로 다음 줄에 __NEEDS_FULL__ 을 단독으로 작성하고
  3) 그 아래에 전체 답변을 이어서 작성해요 (이때도 반드시 완결된 문장으로 끝낼 것)
